...
pixel RayTracer::launchRay(Ray &pRay, uchar level, double nprec)
{
	...
	for (j=0; j<m_scene.m_lights.size(); j++)
	{
		...
		{
			float diff = lightray.m_direction.dotProduct(-Normale) * (m_scene.m_lights[j]->m_coef/* / ((objDist*objDist) / 1000.0f)*/) * FirstObj->m_material->kDiffus*shade;// / (objDist / 100.0f);
			if (diff < 0.0f)
				diff = 0.0f;
			tmpColor.r += ((diff * ((float)m_scene.m_lights[j]->m_color.r / 256.0f) * ((float)FirstObj->m_material->m_color.r / 256.0f)) * 256);
			tmpColor.g += ((diff * ((float)m_scene.m_lights[j]->m_color.g / 256.0f) * ((float)FirstObj->m_material->m_color.g / 256.0f)) * 256);
			tmpColor.b += ((diff * ((float)m_scene.m_lights[j]->m_color.b / 256.0f) * ((float)FirstObj->m_material->m_color.b / 256.0f)) * 256);
		}
		// #2: SPECULAR
		if (FirstObj->m_material->kSpecular > EPSILON)
		{
			Vector vTmp = (lightray.m_direction + pRay.m_direction).getNormal();
			float spec= pow(Normale.dotProduct(vTmp), FirstObj->m_material->kShin) * (m_scene.m_lights[j]->m_coef/* / ((objDist*objDist) / 1000.0f)*/) * FirstObj->m_material->kSpecular*shade;// / (objDist / 100.0f);
			tmpColor.r += ((spec * ((float)m_scene.m_lights[j]->m_color.r / 256.0f)) * 256);
			tmpColor.g += ((spec * ((float)m_scene.m_lights[j]->m_color.g / 256.0f)) * 256);
			tmpColor.b += ((spec * ((float)m_scene.m_lights[j]->m_color.b / 256.0f)) * 256);
		}
	}
	// #3: AMBIENT
	//shade += m_ambient;
